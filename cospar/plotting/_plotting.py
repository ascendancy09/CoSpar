...
def plot_fate_map(adata,selected_fates=[],used_map_name='transition_map',map_backwards=True,plot_time_points=[],plot_background=True,plot_target_state=True,normalize=False,
               auto_color_scale=False,plot_color_bar=True,point_size=2,plot_fate_bias=False,alpha_target=0.2):
...
def plot_differential_genes_for_given_fates(adata_0,selected_fates=[],plot_time_points=[],plot_groups=True,gene_N=100,plot_gene_N=5,savefig=False,point_size=1):
    '''
    ...
    diff_gene_A=[]
    diff_gene_B=[]
    check_transition_map(adata_0)
    set_up_plotting()


    time_info=np.array(adata_0.obs['time_info'])
    if (len(plot_time_points)>0):
        sp_idx=np.zeros(adata_0.shape[0],dtype=bool)
        for xx in plot_time_points:
            sp_id_temp=np.nonzero(time_info==xx)[0]
            sp_idx[sp_id_temp]=True

        adata=adata_0[np.nonzero(sp_idx)[0]]
    else:
        #sp_idx=np.ones(len(cell_id_t1),dtype=bool)
        adata=adata_0
