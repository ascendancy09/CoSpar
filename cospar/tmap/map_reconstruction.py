...
#import cospar.tmap # if we use `import cospar.tmap as tmap`, an error will occur
...
def generate_similarity_matrix(adata,file_name,round_of_smooth=10,neighbor_N=20,beta=0.1,truncation_threshold=0.001,save_subset=True,verbose=True,compute_new_Smatrix=False):
    """
    ...
    if os.path.exists(file_name+f'_SM{round_of_smooth}.npz') and (not compute_new_Smatrix):
        ...
        for iRound in range(round_of_smooth):
            ...
            if verbose:
                print(f"similarity matrix truncated (Smooth round={SM}): ", time.time()-t)
...
def generate_initial_similarity(similarity_matrix,initial_index_0,initial_index_1,verbose=True):
    """
...
def generate_final_similarity(similarity_matrix,final_index_0,final_index_1,verbose=True):
    """
...
def select_time_points(adata_orig,time_point=['day_1','day_2'],verbose=True,use_all_cells=False):
    ...
    if (len(time_point)<2):
        ...
        if verbose:
            ...
            CSpl.scatter_plot_on_embedding(x_emb,y_emb,-x_emb)
...
def refine_Tmap_through_cospar(MultiTime_cell_id_array_t1,MultiTime_cell_id_array_t2,
    ...
    noise_threshold=0.1,normalization_mode=1,verbose=True):
    """
    ...
    if verbose:
        ...
        if normalization_mode==1: print("Clone normalization")
...
def refine_Tmap_through_cospar_noSmooth(MultiTime_cell_id_array_t1,
    ...
    X_clone,noise_threshold=0.1,normalization_mode=1,verbose=True):
    """
    This performs one iteration of coherent sparsity optimization
...
def infer_Tmap_from_multitime_clones(adata_orig,selected_clonal_time_points,
    ...
    verbose=True,trunca_threshold=0.001,compute_new=False):
    """
    ...
    verbose: `bool`, optional (default: True)
        True, print information of the analysis; False, do not print.
    ...
    for xx in selected_clonal_time_points:
        if xx not in adata_orig.uns['clonal_time_points']:
            print(f"Error! 'selected_clonal_time_points' contain time points without clonal information. Please set clonal_time_point to be at least two of {adata_orig.uns['clonal_time_points']}. If there is only one clonal time point, plesae run ----cospar.tmap.infer_Tmap_from_one_time_clones----")
            return adata_orig
    ...
    infer_Tmap_from_multitime_clones_private(adata,smooth_array=smooth_array,neighbor_N=CoSpar_KNN,noise_threshold=noise_threshold,demulti_threshold=demulti_threshold,normalization_mode=normalization_mode,
            save_subset=save_subset,use_full_Smatrix=use_full_Smatrix,verbose=verbose,trunca_threshold=trunca_threshold,compute_new_Smatrix=compute_new)


    return adata
    

def infer_Tmap_from_multitime_clones_private(adata,smooth_array=[15,10,5],neighbor_N=20,noise_threshold=0.1,demulti_threshold=0.05,normalization_mode=1,save_subset=True,use_full_Smatrix=False,verbose=True,trunca_threshold=0.001,compute_new_Smatrix=False):
    """
    ...
    if use_full_Smatrix:
        ...
        for round_of_smooth in smooth_array:
            if not os.path.exists(similarity_file_name+f'_SM{round_of_smooth}.npz'):
                print(f"Error! Similarity matrix at given parameters have not been computed before! Name: {similarity_file_name}")     
                return   
    ...
    for round_of_smooth in smooth_array:
        ...
        if use_full_Smatrix:
            ...
            initial_similarity_ext=generate_initial_similarity(similarity_matrix_full_sp,Tmap_cell_id_t1,clonal_cell_id_t1,verbose=verbose)
            final_similarity_ext=generate_final_similarity(similarity_matrix_full_sp,clonal_cell_id_t2,Tmap_cell_id_t2,verbose=verbose)
            
            ### minimum similarity matrix that only involves the multi-time clones
            initial_similarity=generate_initial_similarity(similarity_matrix_full_sp,clonal_cell_id_t1,clonal_cell_id_t1,verbose=verbose)
            final_similarity=generate_final_similarity(similarity_matrix_full_sp,clonal_cell_id_t2,clonal_cell_id_t2,verbose=verbose)
        else:
            initial_similarity_ext=generate_initial_similarity(similarity_matrix_full,Tmap_cell_id_t1,clonal_cell_id_t1,verbose=verbose)
            final_similarity_ext=generate_final_similarity(similarity_matrix_full,clonal_cell_id_t2,Tmap_cell_id_t2,verbose=verbose)
            initial_similarity=generate_initial_similarity(similarity_matrix_full,clonal_cell_id_t1,clonal_cell_id_t1,verbose=verbose)
            final_similarity=generate_final_similarity(similarity_matrix_full,clonal_cell_id_t2,clonal_cell_id_t2,verbose=verbose)
    ...
    for j in range(CoSpar_iter_N):
        ...
        clonal_coupling, unSM_sc_coupling=refine_Tmap_through_cospar(multiTime_cell_id_t1,multiTime_cell_id_t2,
            proportion,transition_map,X_clone,used_initial_similarity,used_final_similarity,noise_threshold=noise_threshold,verbose=verbose,normalization_mode=normalization_mode)
...
def infer_intraclone_Tmap(adata,demulti_threshold=0.05,normalization_mode=1):
    ...
    if 'transition_map' not in adata.uns.keys():
        print("Please run ---- CS.tmap.infer_Tmap_from_multitime_clones ---- first")
...
def Tmap_from_highly_variable_genes_v1(adata,min_counts=3,min_cells=3,
    ...
    verbose=True,trunca_threshold=0.001,compute_new_Smatrix=True,
    save_subset=True):
    """
    ...
    print("HighVar-v1: do not avoid cells that have been selected")
...
def Tmap_from_highly_variable_genes(adata,min_counts=3,min_cells=3,
    ...
    None. Results are stored at adata.uns['HighVar_transition_map']. 
    """
    print("HighVar-v0: avoid cells that have been selected")
    weight=1 # wehight of each gene. 
...
def compute_custom_OT_transition_map(adata,OT_epsilon=0.02,OT_dis_KNN=5,
    OT_solver='duality_gap',OT_cost='SPD',verbose=True,compute_new=True):
    """
    ...
    if OT_cost=='SPD':
        ...
        if verbose:
            print(f"Finishing computing gene expression distance, used time {time.time()-t}")  
            
    # if verbose: # for checking the quality of the shortest path distance.
    #     target_cell_ID=2;
...
#             print("Use sinkhorn solver solver")
...
def compute_custom_OT_transition_map_v0(adata,OT_epsilon=0.02,OT_max_iter=1000,
    OT_stopThr=1e-09,OT_dis_KNN=5,OT_method='OT',verbose=True,compute_new=True):
    """
    Compute Tmap from state info using optimal transport (OT).
...
# A test to see whether vectorization improves the speed. v2
def infer_Tmap_from_one_time_clones_private(adata,initialized_map,Clone_update_iter_N=1,
    ...
    use_full_Smatrix=False,verbose=True,trunca_threshold=0.001,compute_new=True,
    ...
    if verbose:
        print("Joint optimization that consider possibility of clonal overlap: v2")
    ...
    if verbose:
        print(f"original clone shape: {X_clone.shape}")
        print(f"After excluding zero-sized clones: {X_clone_temp.shape}")
    ...
    for x0 in range(Clone_update_iter_N):
        ...
        for j in range(clone_N1):
            ...
            # infer the earlier clonal states for each clone
            ### select the early states using the grouped distribution of a clone
            sorted_id_array=np.argsort(initial_prob_matrix[remaining_ids_t1,j])[::-1]

            cell_N_to_extract_0=clone_size_t2[j]*len(cell_id_array_t1)/clonal_cells_t2
            ...
            sel_id_t1=sorted_id_array[:cell_N_to_extract]
            ...
            if len(remaining_ids_t1)==0: 
                print(f'Early break. Final clone id: {j+1}')
                break
        ...
        adata.obsm['X_clone']=ssp.csr_matrix(X_clone_new)*(clone_mapping.T) # convert back to the original clone structure
        ...
        infer_Tmap_from_multitime_clones_private(adata,smooth_array=smooth_array,neighbor_N=CoSpar_KNN,noise_threshold=noise_threshold,
            normalization_mode=normalization_mode,save_subset=True,use_full_Smatrix=use_full_Smatrix,verbose=verbose,
...
# This is the current version, v1. This is good, but time consuming. 
...
def infer_Tmap_from_one_time_clones_private_v0(adata,initialized_map,Clone_update_iter_N=1,
    ...
    smooth_array=[15,10,5],verbose=True,trunca_threshold=0.001,compute_new=True,
    use_fixed_clonesize_t1=False,sort_clone=1):
    """
    Infer Tmap from clones with a single time point
...
def infer_Tmap_from_one_time_clones(adata_orig,initial_time_points,clonal_time_point,
    ...
    * Choose the initialization method, and set the corresponding parameters. 
        ...
        * 'HighVar':  is faster and robust to batch effect.
          Key parameter: `HighVar_gene_pctl`.
    
        * 'OT': tend to be more accurate, but very slow and not reliable 
    ...
    * Key parameters relevant for CoSpar itself: `smooth_array, normalization_mode, 
    ...
    OT_max_iter: `int`, optional (default: 1000)
        Maximum round of iteration. Relevant when `initialize_method='OT'`.
    OT_stopThr: `float`, optional (default: 1e-09)
        Stop if the cost function change per iteration is 
        less than this threshold. Relevant when `initialize_method='OT'`.
    OT_dis_KNN: `int`, optional (default: 5)
        Number of nearest neighbors to construct the KNN graph for
        computing the shortest path distance. Relevant when `initialize_method='OT'`. 
    OT_method: `str`, optional (default: 'OT')
        The method to compute the optimal transport map: {'OT','WOT'}
    ...
    for yy in initial_time_points:
        ...
        adata_temp=infer_Tmap_from_one_time_clones_twoTime(adata_orig,selected_two_time_points=[yy,clonal_time_point],
            ...
            verbose=verbose,trunca_threshold=trunca_threshold,compute_new=compute_new,
...
def infer_Tmap_from_one_time_clones_twoTime(adata_orig,selected_two_time_points=['1','2'],
    ...
    verbose=True,trunca_threshold=0.001,compute_new=True,use_fixed_clonesize_t1=False,
    ...
    if (N_valid_time!=2): 
        ...
        if use_full_Smatrix: 
            ...
            if not (os.path.exists(similarity_file_name+f'_SM{round_of_smooth}.npz') and (not compute_new)):
                similarity_matrix_full=generate_similarity_matrix(adata_orig,similarity_file_name,round_of_smooth=round_of_smooth,
                            neighbor_N=CoSpar_KNN,truncation_threshold=trunca_threshold,save_subset=save_subset,verbose=verbose,compute_new_Smatrix=compute_new)
        ...
        if initialize_method=='OT':
            ...
            compute_custom_OT_transition_map(adata,OT_epsilon=OT_epsilon,OT_cost=OT_cost,OT_dis_KNN=OT_dis_KNN,verbose=verbose,compute_new=compute_new)
            ...
            Tmap_from_highly_variable_genes(adata,min_counts=3,min_cells=3,min_gene_vscore_pctl=HighVar_gene_pctl,noise_threshold=noise_threshold,neighbor_N=CoSpar_KNN,
                normalization_mode=normalization_mode,use_full_Smatrix=use_full_Smatrix,smooth_array=smooth_array,verbose=verbose,trunca_threshold=trunca_threshold,
        ...
        if selected_two_time_points[1] in adata_orig.uns['clonal_time_points']:
            ...
            infer_Tmap_from_one_time_clones_private(adata,initialized_map,Clone_update_iter_N=Clone_update_iter_N,normalization_mode=normalization_mode,noise_threshold=noise_threshold,
                CoSpar_KNN=CoSpar_KNN,use_full_Smatrix=use_full_Smatrix,smooth_array=smooth_array,verbose=verbose,trunca_threshold=trunca_threshold,
...
def infer_weinreb_Tmap(adata):
    ...
    print("This method works when there are only time points and all datasets")
    cell_id_t2=adata.uns['Tmap_cell_id_t2']
    cell_id_t1=adata.uns['Tmap_cell_id_t1']
    clone_annot=adata.obsm['X_clone']
