...
def refine_clonal_map_by_integrating_clonal_info(cell_id_array_t1,cell_id_array_t2,clonal_map,cell_by_clone_matrix,initial_kernel,final_kernel,noise_threshold,row_normalize=True,verbose=True,normalization_mode=2):
...
def refine_clonal_map_by_integrating_clonal_info_multiTime(MultiTime_cell_id_array_t1,MultiTime_cell_id_array_t2,proportion,clonal_map,cell_by_clone_matrix,initial_kernel,final_kernel,noise_threshold,row_normalize=True,verbose=True,normalization_mode=2):
...
def refine_clonal_map_by_integrating_clonal_info_multiTime_No_SM(MultiTime_cell_id_array_t1,MultiTime_cell_id_array_t2,proportion,clonal_map,cell_by_clone_matrix,intra_clone_threshold=0,noise_threshold=0.1,normalization_mode=2,verbose=True):
...
def CoSpar_TwoTimeClones(adata_orig,selected_clonal_time_points=['1','2'],SM_array=[15,10,5],CoSpar_KNN=20,noise_threshold=0.1,demulti_threshold=0.05,normalization_mode=2,use_all_cells=False,save_subset=True,use_full_kernel=False,verbose=True,trunca_threshold=0.001,compute_new=False):
    '''
        Purpose: 
...
def CoSpar_TwoTimeClones_private(adata,SM_array=[15,10,5],neighbor_N=20,noise_threshold=0.1,demulti_threshold=0.05,normalization_mode=2,save_subset=True,use_full_kernel=False,verbose=True,trunca_threshold=0.001,compute_new_Smatrix=False):
    '''
...
def CoSpar_TwoTimeClones_demultiplexing(adata,intra_clone_threshold=0,demulti_threshold=0.05,normalization_mode=2):
    '''
...
def Transition_map_from_highly_variable_genes(adata,min_counts=3,min_cells=3,min_vscore_pctl=85,noise_threshold=0.2,neighbor_N=20,normalization_mode=2,
...
def CoSpar_OneTimeClone_JointOptimization_Private(adata,initialized_map,Clone_update_iter_N=1,normalization_mode=2,
    noise_threshold=0.2,CoSpar_KNN=20,use_full_kernel=False,SM_array=[15,10,5],verbose=True,trunca_threshold=0.001,compute_new=True,use_fixed_clonesize_t1=False,sort_clone=1):
    '''

        sort_clone=1 #0,not sort; 1 sort from small to large; -1 sort from large to small
        If use sort_clone=1 and use_fixed_clonesize_t1=True, it goes back to our previous methods
...
def CoSpar_OneTimeClone_JointOptimization_Private_v0(adata,initialized_map,Clone_update_iter_N=1,normalization_mode=2,
...
def CoSpar_OneTimeClones(adata_orig,initial_time_points=['1'],clonal_time_point='2',initialize_method='OT',OT_epsilon=0.02,OT_dis_KNN=5,OT_max_iter=1000,OT_stopThr=1e-09,HighVar_gene_pctl=85,
    Clone_update_iter_N=1,normalization_mode=2,noise_threshold=0.2,CoSpar_KNN=20,use_full_kernel=False,SM_array=[15,10,5],
    verbose=True,trunca_threshold=0.001,compute_new=False,use_fixed_clonesize_t1=False,sort_clone=1):
...
def CoSpar_OneTimeClones_TwoTimePoints(adata_orig,selected_two_time_points=['1','2'],initialize_method='OT',OT_epsilon=0.02,OT_dis_KNN=5,OT_max_iter=1000,OT_stopThr=1e-09,HighVar_gene_pctl=80,
    Clone_update_iter_N=1,normalization_mode=2,noise_threshold=0.2,CoSpar_KNN=20,use_full_kernel=False,SM_array=[15,10,5],
    verbose=True,trunca_threshold=0.001,compute_new=True,use_fixed_clonesize_t1=False,sort_clone=1):
...
def compute_weinreb_map(adata):
    ...
    cell_id_t1=adata.uns['Tmap_cell_id_t1']
    clone_annot=adata.obsm['cell_by_clone_matrix']
    state_annote=np.array(adata.obs['state_annotation'])
