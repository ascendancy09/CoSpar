...
def plot_fate_bias_SW(adata,x0,y0,fate_names,plot_name):
    N_cell, N_gene=adata.shape
    cell_fate_num=np.zeros(N_cell)
    N_fate=len(fate_names)
    for j in range(N_cell):
        cell_fate_num[j]=sum(adata.var_vector(j)[(N_gene-N_fate):N_gene])
        
    
    nrow=int(np.ceil(N_fate/3))
    fig = plt.figure(figsize = (12, 2.5 * nrow))
    
    ## plot a single cell fate
    for j,fate_ID in enumerate(fate_names):
        ax0 = plt.subplot(nrow, 5, j+1)
        idx=(adata.obs_vector(fate_ID)>0) & (cell_fate_num==1) 
        plot_one_gene_SW(x0,y0,idx,ax=ax0)
        plt.title(fate_ID)

    ## plot two cell fates    
#     Mono_Dc=np.zeros(sum(selected_cell_idx));
#     Mono_Neu=np.zeros(sum(selected_cell_idx));
#     Ma_Ba=np.zeros(sum(selected_cell_idx));
#     Neu_Ba=np.zeros(sum(selected_cell_idx));
#     Mono_id=find_nonzero_index_SW([fate_id=='Mono' for fate_id in fate_names])[0]
#     Neu_id=find_nonzero_index_SW([fate_id=='Neu' for fate_id in fate_names])[0]
#     Dc_id=find_nonzero_index_SW([fate_id=='Dc' for fate_id in fate_names])[0]
#     Lym_id=find_nonzero_index_SW([fate_id=='Lym' for fate_id in fate_names])[0]
#     Ba_id=find_nonzero_index_SW([fate_id=='Ba' for fate_id in fate_names])[0]
#     Ma_id=find_nonzero_index_SW([fate_id=='Ma' for fate_id in fate_names])[0]
    
    
    Mono_Dc=(cell_fate_num==2) & (adata.obs_vector('Mono')>0) & (adata.obs_vector('Dc')>0)
    Mono_Neu=(cell_fate_num==2) & (adata.obs_vector('Mono')>0) & (adata.obs_vector('Neu')>0)
    Neu_Ba=(cell_fate_num==2) & (adata.obs_vector('Neu')>0) & (adata.obs_vector('Ba')>0)
    Mono_Dc_Neu=(cell_fate_num==3) & (adata.obs_vector('Neu')>0) & (adata.obs_vector('Mono')>0) & (adata.obs_vector('Dc')>0)
    Mono_Neu_Ba=(cell_fate_num==3) & (adata.obs_vector('Neu')>0) & (adata.obs_vector('Mono')>0) & (adata.obs_vector('Ba')>0) 
    Mk_Ma_Ba=(cell_fate_num==3) & (adata.obs_vector('Mk')>0) & (adata.obs_vector('Ma')>0) & (adata.obs_vector('Ba')>0) 
                
#     for k in range(len(Mono_Dc)):
#         temp=adata.var_vector(k)[len(highvar_genes_1):N_gene]
#         if (sum(temp)==2) and (temp[Mono_id]==1) and (temp[Neu_id]==1):
#             Mono_Neu[k]=1
#         if (sum(temp)==2) and (temp[Mono_id]==1) and (temp[Dc_id]==1):
#             Mono_Dc[k]=1
#         if (sum(temp)==2) and (temp[Ma_id]==1) and (temp[Ba_id]==1):
#             Ma_Ba[k]=1
#         if (sum(temp)==2) and (temp[Neu_id]==1) and (temp[Ba_id]==1):
#             Neu_Ba[k]=1


    ax0 = plt.subplot(nrow, 5, j+2)
    plot_one_gene_SW(x0,y0,Mono_Dc>0,ax=ax0)
    plt.title("Mono_Dc")
    ax0 = plt.subplot(nrow, 5, j+3)
    plot_one_gene_SW(x0,y0,Mono_Neu>0,ax=ax0)
    plt.title("Mono_Neu")
    ax0 = plt.subplot(nrow, 5, j+4)
    plot_one_gene_SW(x0,y0,Neu_Ba>0,ax=ax0)
    plt.title("Neu_Ba")
    ax0 = plt.subplot(nrow, 5, j+5)
    plot_one_gene_SW(x0,y0,Mk_Ma_Ba,ax=ax0)
    plt.title("Mk_Ma_Ba")
    ax0 = plt.subplot(nrow, 5, j+6)
    plot_one_gene_SW(x0,y0,Mono_Neu_Ba,ax=ax0)
    plt.title("Mono_Neu_Ba")
    ax0 = plt.subplot(nrow, 5, j+7)
    plot_one_gene_SW(x0,y0,Mono_Dc_Neu,ax=ax0)
    plt.title("Mono_Dc_Neu")
...
def plot_fate_bias_SW_v2(clonal_info,x0,y0,fate_names,plot_name):
    N_cell=clonal_info.shape[0]
    cell_fate_num=np.zeros(N_cell)
    N_fate=len(fate_names)
    cell_fate_num=np.sum(clonal_info,1)
    ...
    for j,fate_ID in enumerate(fate_names):
        ax0 = plt.subplot(nrow, 5, j+1)
        idx=(clonal_info[:,j]>0) & (cell_fate_num==1) 
        plot_one_gene_SW(x0,y0,idx,ax=ax0)
        plt.title(fate_ID)
...
#     Neu_Ba=np.zeros(sum(selected_cell_idx));
    ...
    Ba_id=find_nonzero_index_SW([fate_id=='Ba' for fate_id in fate_names])[0]
    Ma_id=find_nonzero_index_SW([fate_id=='Ma' for fate_id in fate_names])[0]
    Mk_id=find_nonzero_index_SW([fate_id=='Mk' for fate_id in fate_names])[0]
    
    
    Mono_Dc=(cell_fate_num==2) & (clonal_info[:,Mono_id]>0) & (clonal_info[:,Dc_id]>0)
    Mono_Neu=(cell_fate_num==2) & (clonal_info[:,Mono_id]>0) & (clonal_info[:,Neu_id]>0)
    Neu_Ba=(cell_fate_num==2) & (clonal_info[:,Neu_id]>0) & (clonal_info[:,Ba_id]>0)
    Mono_Dc_Neu=(cell_fate_num==3) & (clonal_info[:,Neu_id]>0) & (clonal_info[:,Mono_id]>0) & (clonal_info[:,Dc_id]>0)
    Mono_Neu_Ba=(cell_fate_num==3) & (clonal_info[:,Neu_id]>0) & (clonal_info[:,Mono_id]>0) & (clonal_info[:,Ba_id]>0) 
    Mk_Ma_Ba=(cell_fate_num==3) & (clonal_info[:,Mk_id]>0) & (clonal_info[:,Ma_id]>0) & (clonal_info[:,Ba_id]>0) 
...
def classifier_learn_pca_projection(adata_ref, highvar_genes_column='highly_variable_projection', cell_normalize=True, find_highvar_genes=True, mean_center=True, n_pcs=30, n_neighbors=10, knn_metric='euclidean', random_state=1, **kwargs):
    ...
    if cell_normalize:
        # depending on sequencing depth, each sample may have a different mean. So, we better to aim for a constant UMI per cell
        sc.pp.normalize_per_cell(adata_tmp, counts_per_cell_after=1e4)
        #sc.pp.normalize_per_cell(adata_tmp, counts_per_cell_after=adata_tmp.obs['n_counts'].mean())
...
def get_pca_SW(E, scaling=1, ini_index=1, base_ix=[], numpc=50, keep_sparse=False, normalize=True, random_state=0):
    '''
    This version is tailored to the sc Projection problem. 
    Note that there is Zscore transformation before PCA. this may not be what you want
    ...
    if keep_sparse:
        ...
        if normalize:
            ...
            Z = E
        for j in range(ini_index,N_gene):
            Z[:,j]=Z[:,j]*scaling
...
def clustering_score_SW(clustered_data,knn_indices):
    '''
    We assume the clustrered_data is a cell-by-cluster_id matrix, and each cell can only occupy one cluster. If a given cell has a positive value at a cluster column, it is deemed as belonging to this cluster. 
    '''
    N_cell_1,N_clusters=clustered_data.shape
    match_matrix=np.zeros((N_cell_1,N_clusters))
    for j in range(N_cell_1):
        orig=clustered_data[j,:]
        orig_index=find_nonzero_index_SW(orig)[0]
        match_matrix[orig_index,0]+=1
        neighber_index=knn_indices[j]
        for m in range(1,len(neighber_index)):
            k=neighber_index[m]
            neib=clustered_data[k,:]
            neib_index=find_nonzero_index_SW(neib)[0]
            if neib_index==orig_index:
                match_matrix[orig_index,1]+=1 #record the event of match, but not mismatch

    match_score=np.zeros(N_clusters)
    for j in range(N_clusters):
        match_score[j]=match_matrix[j,1]/(N_clusters*match_matrix[j,0])
        
    return match_score

def find_nonzero_index_SW(list):
    '''
...
def get_smooth_values(input_values, adjacency_matrix, beta=0.1, n_rounds=10):
    ...
    # Get knn edges
    edges = get_knn_graph(adata.obsm['X_pca'], k=10, return_edges=True)[0]

    # Build adjacency matrix from edges
    adjacency_matrix = build_adj_mat(edges, adata.shape[0])
...
def get_smooth_values_SW(input_values, adjacency_matrix, beta=0.1, n_rounds=10):
    ''' Smooth values on a graph
    ...
    if len(input_values.shape) == 1:
        smoothed_values = input_values[:, None].T
    else:
        smoothed_values = input_values.copy()
        smoothed_values=smoothed_values.T;


    if ssp.issparse(adjacency_matrix):  
        adjacency_matrix = sparse_rowwise_multiply(adjacency_matrix, 1 / adjacency_matrix.sum(1).A.squeeze())
    else:
        adjacency_matrix = sparse_rowwise_multiply(adjacency_matrix, 1 / adjacency_matrix.sum(1))
        
    for iRound in range(n_rounds):
        smoothed_values = (beta * smoothed_values + ((1 - beta) * smoothed_values*adjacency_matrix))

    smoothed_values=smoothed_values.T
    if ssp.issparse(smoothed_values):  
        return smoothed_values.A.squeeze()
    else: 
        return smoothed_values.squeeze()


def get_smooth_values_sparseMatrixForm(input_values, adjacency_matrix, beta=0.1, n_rounds=10):
    ''' Smooth values on a graph
    
        We expect the input "input_values" to be a sparse matrix, and also the output
    '''
    ...
    return smoothed_values.T


########## CLUSTERING

def select_cells_for_plotting_SW(cluster_ID,count_array_max,label):
    # count_array_max is the maximum number of cells to be generated for each cluster
    N_cluster=len(count_array_max);
#    cluster_ID=adata_corr.obs['leiden_1'].astype(int);
    count_array=np.zeros(N_cluster);
    selected_cells=np.zeros(len(cluster_ID))
    for j in range(len(cluster_ID)):
        if count_array[cluster_ID[j]]<=count_array_max[cluster_ID[j]]:
            selected_cells[j]=label; # selected
            count_array[cluster_ID[j]]=count_array[cluster_ID[j]]+1; # update
...
def get_dge_SW(ad, mask1, mask2, min_frac_expr=0.05, pseudocount=1):
    ...
    df = pd.DataFrame({
        ...
        'pv': pv[sort_idx],
        'm1': m1[sort_idx] - pseudocount, 
        'm2': m2[sort_idx] - pseudocount, 
        'ratio': r[sort_idx]
    })
...
def save_hdf5_genes(E, gene_list, filename):
    ...
    counts_group = create_group('counts')
    cix_group = create_group('cell_ix')

    attrs['ncells'] = E.shape[0]
    attrs['ngenes'] = E.shape[1]
...
def save_hdf5_cells(E, filename):
    ...
    hf = h5py.File(filename, 'w')
    counts_group = create_group('counts')
    gix_group = create_group('gene_ix')
...
## to order to plots according to the group label, assumed to be numerical        
def plot_single_group_SW(x, y, groups, buffer_pct=0.03, ax=None,saving=False, fig_dir='./', fig_name='fig', res=300, close_after=False, title_size=12, point_size=3, text_size=10,ncol=1,with_label=0):
    import matplotlib.pyplot as plt

    # n_col = 1
    # ngroup = 1
    # nrow = int(np.ceil(ngroup / float(ncol)))
    # fig = plt.figure(figsize = (4, 3 * nrow))
    ...
    point_color=[[0,0,1]]
    if with_label:
        for j in range(sum(ix)):
            ax.scatter(x1[j], y1[j], s=point_size, c=point_color, edgecolors = '')
            ax.text(x1[j],y1[j],str(j),fontsize=text_size)
    else:
        ax.scatter(x[ix], y[ix], s=point_size, c=point_color, edgecolors = '')
    ...
    ax.set_ylim(y.min()-y.ptp()*buffer_pct, y.max()+y.ptp()*buffer_pct)
    ax.axis('off')
    #ax.set_title(str(ii), fontsize = title_size)
...
def generate_regular_lattice_SW(x_umap,y_umap,mini_dis=0.1,initial_sp_ratio=0.5):
    '''
        The initial_sp_ratio can be lower if the total cell number is very large;
        Otherwise, it should be close to 1. 

        The mini_dis is is a relative fraction, in terms of the largest distance in the data 
    '''

    cell_N=len(x_umap);
    rand_nodes_0=np.random.random(cell_N)<initial_sp_ratio;
    count=0;
    rand_nodes=np.zeros(cell_N);
    x_array=x_umap[rand_nodes_0];
    y_array=y_umap[rand_nodes_0];
    max_dis=max(max(x_array),max(y_array))
    for j in range(cell_N):
        if rand_nodes_0[j]>0:
            if count==0:
                ...
                new_idx=np.zeros(cell_N)
                new_idx[0:j+1]=1;
                x_array_1=x_umap[(rand_nodes*new_idx)>0];
                y_array_1=y_umap[(rand_nodes*new_idx)>0];
                shift_x=x_array_1-x_umap[j]
                shift_y=y_array_1-y_umap[j]
                potential_index=(abs(shift_x)<mini_dis*max_dis) & (abs(shift_y)<mini_dis*max_dis)
                #print(sum(potential_index))
                if sum(potential_index)==0: # this node is too close to existing nodes
                    rand_nodes[j]=1; # remove this node
                    #print(j)
                    count=count+1;
    rand_nodes=rand_nodes>0;
    return rand_nodes
        
    
    
def plot_graph_SW(adata_corr,matrix,position,make_transform=0,threshold=0, title='', arrowsize=1, edge_width_scale=0.1,
     fontsize=0,max_edge_width=3, frameon=False,  save=True,solid_edges='connectivities',transitions='transitions_confidence'):
    '''
       Utilize the plot function in Paga to plot a graph "matrix". The embedding is controlled by 'pos' 
    '''

    N0=matrix.shape[0]
    inferred_rates=np.zeros((N0,N0))
    if make_transform:
        # rate normalization
        max_R=np.max(matrix)
        inferred_rates=matrix
        for j in range(N0):
            if max(matrix[j,:])>1: # if the total outgoing rate is larger than a certain threshold
                inferred_rates[j,:]=matrix[j,:]/max(matrix[j,:])
    else:
        inferred_rates=matrix

    cell_N=adata_corr.X.shape[0]
    cell_cluster=np.zeros(cell_N)
    cell_cluster[0:N0]=np.array(range(N0))
    adata_corr.obs['leiden_2']=pd.Categorical(cell_cluster.astype(int))
    adata_corr.uns['paga']['connectivities']=ssp.csr_matrix(inferred_rates.T)
    adata_corr.uns['paga']['connectivities_tree']=ssp.csr_matrix(inferred_rates.T)
    adata_corr.uns['paga']['transitions_confidence']=ssp.csr_matrix(inferred_rates.T)
    adata_corr.uns['paga']['groups']='leiden_2'; # direct paga where to find the color matrix
    adata_corr.uns['leiden_2_colors']=['#FF0000' for j in range(N0)]
    #sc.pl.paga(adata_corr,color='leiden_1',pos=sp_X)

    sc.pl.paga(adata_corr,color='leiden_2',pos=position,threshold=threshold, title=title, arrowsize=arrowsize, edge_width_scale=edge_width_scale,
         fontsize=fontsize,max_edge_width=max_edge_width, frameon=frameon,  save=save,solid_edges=solid_edges,transitions=transitions)



## to order plots according to the group label, assumed to be numerical        
def plot_groups_SW(x, y, groups, buffer_pct=0.03, saving=False, fig_dir='./', fig_name='fig', res=300, close_after=False, title_size=12, point_size=3, ncol=5):
    import matplotlib.pyplot as plt

    n_col = int(ncol)
    ngroup = len(np.unique(groups))
    nrow = int(np.ceil(ngroup / float(ncol)))
    fig = plt.figure(figsize = (18, 3 * nrow))
    max_N=max(groups)+1;
    for ii in range(max_N):
        ...
        ax.scatter(x[~ix], y[~ix], s=point_size, c=[[.8,.8,.8]], edgecolors = '')
        ax.scatter(x[ix], y[ix], s=point_size, c=[[0,0,1]], edgecolors = '')
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_xlim(x.min()-x.ptp()*buffer_pct, x.max()+x.ptp()*buffer_pct)
        ax.set_ylim(y.min()-y.ptp()*buffer_pct, y.max()+y.ptp()*buffer_pct)
...
def plot_cluster_size_SW(groups):
    max_N=max(groups)+1;
    for ii in range(max_N):
        print(sum(groups==ii));

        
def spreading_SW(x,y,Prob):
    temp=sum(Prob);
    if temp>0:
        Prob=Prob/sum(Prob);
        x_ave=np.sum(x*Prob);
        y_ave=np.sum(y*Prob);
        spreading=np.sum((x-x_ave)*(x-x_ave)*Prob+(y-y_ave)*(y-y_ave)*Prob);
        return spreading
    else:
        return 0
        
def plot_one_gene_SW(x, y, vector, normalize=False, title=None, ax=None, order_points=True, set_ticks=0, col_range=(0, 100), buffer_pct=0.03, point_size=1, color_map=None, smooth_operator=None,savefig=False,dpi=300,set_lim=True,vmax=np.nan,vmin=np.nan):
...
def plot_categorical_SW(x, y, data, buffer_pct=0.03, point_size=5, color_map=None):
    ...
    row = int(np.ceil(group_labels.shape[0]/4))
    col = 4
    fig = plt.figure(figsize=(4 * col, 4 * row))

    for j,label in enumerate(group_labels):
        target_cell_ID=(data==label)
        ax0 = plt.subplot(row, col, j+1)
        plot_one_gene_SW(x,y,target_cell_ID,point_size=point_size,ax=ax0)
        ax0.set_title(label)
        ax0.set_xticks([])
        ax0.set_yticks([])
        ax0.set_xlim(x.min()-x.ptp()*buffer_pct, x.max()+x.ptp()*buffer_pct)
        ax0.set_ylim(y.min()-y.ptp()*buffer_pct, y.max()+y.ptp()*buffer_pct)
